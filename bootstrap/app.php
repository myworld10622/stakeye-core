...
try {
    ...
    $reqPathTrim = ltrim($reqPath, '/');

    // Allow activation UI and common public asset prefixes
    $allowPrefixes = [
        'activate',      // activation UI & subroutes
        'assets',        // assets folder
        '_debugbar',     // debugbar assets
        'javascript',    // some asset endpoints
        'js', 'css',     // common static paths
        'images', 'img', 'fonts', 'favicon', 'robots.txt'
    ];

    foreach ($allowPrefixes as $prefix) {
        if ($reqPathTrim === $prefix || str_starts_with($reqPathTrim, $prefix . '/')) {
           if (defined('FORCE_ACTIVATED') && FORCE_ACTIVATED === true) {
    return $next($request);
}
            break;
        }
...
return Application::configure(basePath: dirname(__DIR__))
        ...
        using:function(){
            Route::namespace('App\Http\Controllers')->middleware([VugiChugi::mdNm()])->group(function(){
                ...
                Route::middleware(['web','maintenance'])->prefix('user')->group(base_path('routes/user.php'));
                       Route::middleware(['web','maintenance'])->prefix('games')->group(base_path('routes/games.php'));
    ...
    ->withMiddleware(function (Middleware $middleware) {
        ...
        $middleware->alias([
            ...
            'check.api.origin' => CheckApiOrigin::class,
        ]);
