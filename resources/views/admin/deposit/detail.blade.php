                ...
                <div class="card-body">
                    ...
                    <ul class="list-group">
                        ...
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                          ...
                          <li class="list-group-item d-flex justify-content-between align-items-center">
                             @lang('Bonus Included?')
                          @if(!empty($deposit->reward_id))
                                            <span class="badge badge--success">
                                                {{ $deposit->getRewardBonus->name ?? 'N/A' }}
                                            </span>
                                        @else
                ...
                <div class="card-body">
                    ...
                    @if($deposit->status == Status::PAYMENT_PENDING)
                        ...
                        @if(!empty($deposit->reward_id))
                            <div class="row mt-4">
                            <div class="col-md-12">
                            <h4> User have selected a bonus</h4>
                                Name : {{ $deposit->getRewardBonus->name ?? 'N/A' }}<br/>
                                Type : {{ $deposit->getRewardBonus->type ?? 'N/A' }}<br/>
                                Min deposit : {{ $deposit->getRewardBonus->min_deposit_amount ?? 'N/A' }}<br/>
                                Max deposit : {{ $deposit->getRewardBonus->max_deposit_amount ?? 'N/A' }}<br/>
                                Conversion Type : {{ $deposit->getRewardBonus->conversion_type ?? 'N/A' }}<br/>
                                Amount : {{ $deposit->getRewardBonus->amount ?? 'N/A' }}<br/>
                                Max Amount Allowed : {{ $deposit->getRewardBonus->max_amount_allowed ?? 'N/A' }}<br/>
                                For First Deposit : {{ $deposit->getRewardBonus->for_first_deposit==1 ? 'Yes' : 'No' }}<br/>
                                Timeline in Days : {{ $deposit->getRewardBonus->timeline_in_days ?? 'N/A' }}<br/>
                                PNL Required Multiplier : {{ $deposit->getRewardBonus->pnl_required_multiplier ?? 'N/A' }}<br/>
                        
                              <button class="btn btn-outline--success btn-sm ms-1 confirmationBtn"
                                data-action="{{ route('admin.deposit.approve', ['id' => $deposit->id, 'with_bonus' => 1]) }}"
                                data-question="@lang('Are you sure to approve this transaction with bonus?')"
                                ><i class="las la-check"></i>
                                    @lang('Approve & Gave Bonus')
                                </button>
                            </div>
                        </div>
