                            ...
                            @forelse ($crons as $cron)
                                ...
                                <tr>
                                    ...
                                    <td>@if($cron->next_run)
                                        ...
                                        @endif</td>
                                    <td>@if($cron->last_run)
                                            {{ __($cron->last_run) }} <br> {{ diffForHumans($cron->last_run) }}
                                        @else
                                            --
                                        @endif</td>
                                    <td>
                                            ...
                                            <div class="dropdown-menu p-0">
                                                <a href="{{ route('cron') }}?alias={{$cron->alias}}"
                                                   class="dropdown-item"><i
                                                        class="las la-check-circle"></i> @lang('Run Now')</a>
                                                @if($cron->is_running)
                                                    <a href="{{ route('admin.cron.schedule.pause', $cron->id) }}"
                                                       class="dropdown-item"><i class="las la-pause"></i> @lang('Pause')
                                                    </a>
                                                @else
                                                    <a href="{{ route('admin.cron.schedule.pause', $cron->id) }}"
                                                       class="dropdown-item"><i class="las la-play"></i> @lang('Play')
                                                    </a>
                                                @endif
                                                <a href="" data-id="{{ $cron->id }}" data-name="{{ $cron->name }}"
                                                   data-url="{{ $cron->url }}" data-next_run="{{ $formattedDateTime }}"
                                                   data-cron_schedule_id="{{ $cron->cron_schedule_id }}"
                                                   data-default="{{ $cron->is_default }}"
                                                   class="dropdown-item updateCron"><i
                                                        class="las la-pen"></i> @lang('Edit')</a>
                                                <a href="{{ route('admin.cron.schedule.logs', $cron->id) }}"
                                                   class="dropdown-item"><i class="las la-history"></i> @lang('Logs')
                                                </a>
                                                @if(!$cron->is_default)
