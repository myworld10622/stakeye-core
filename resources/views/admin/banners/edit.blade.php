    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{route('admin.frontend.banner.update',$slider->id)}}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control @error('category') is-invalid @enderror" id="category" name="category" required>
                                @foreach($categories as $key => $category)
                                    <option value="{{ $key }}" {{ old('category', $slider->category) == $key ? 'selected' : '' }}>{{ $category }}</option>
                                @endforeach
                            </select>
                            @error('category')
                                <span class="invalid-feedback" role="alert">
                        ...
                        <div class="form-group">
                            ...
                            <img src="{{ getImage(getFilePath('lottery') . '/' . @$slider->image_path, getFileSize('lottery')) }}" alt="{{ $slider->title }}" style="max-width: 200px; margin-bottom: 10px;">
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="url" class="form-control @error('link_url') is-invalid @enderror" id="link_url" name="link_url" value="{{ old('link_url', $slider->link_url) }}" required>
                            @error('link_url')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control @error('players_count') is-invalid @enderror" id="players_count" name="players_count" value="{{ old('players_count', $slider->players_count) }}">
                            @error('players_count')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control @error('sort_order') is-invalid @enderror" id="sort_order" name="sort_order" value="{{ old('sort_order', $slider->sort_order) }}">
                            @error('sort_order')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
